...
const RATE_LIMIT_CONFIG = {
  ...
  // Permanent lockout configuration
  permanentLockoutThreshold: 10,
  permanentLockoutWindowHours: 24,
} as const;
...
export async function handle(request: Request) {
  try {
    ...
    const permanentLockoutWindowStart = new Date(
      now.getTime() - RATE_LIMIT_CONFIG.permanentLockoutWindowHours * 60 * 60 * 1000
    );
    ...
    const result = await db.transaction().execute(async (trx) => {
      ...
      // Check for permanent lockout first - count failed attempts in the last 24 hours
      const permanentLockoutCheck = await trx
        ...
        .where("success", "=", false)
        .where("attemptedAt", ">=", permanentLockoutWindowStart)
        .where("attemptedAt", "is not", null)
        .executeTakeFirst();

      const permanentLockoutFailedCount = permanentLockoutCheck?.failedCount || 0;

      // If permanently locked out, reject immediately without logging attempt
      if (permanentLockoutFailedCount >= RATE_LIMIT_CONFIG.permanentLockoutThreshold) {
        console.warn(`Permanent lockout triggered for email: ${normalizedEmail}. Failed attempts in 24h: ${permanentLockoutFailedCount}`);
        return {
          type: "permanently_locked" as const,
        };
      }

      // Get rate limiting info efficiently for temporary lockout - use COUNT and MAX instead of SELECT *
    ...
    if (Math.random() < RATE_LIMIT_CONFIG.cleanupProbability) {
      ...
      try {
        ...
        if (deleteResult.numDeletedRows) {
          console.log(`Cleaned up ${deleteResult.numDeletedRows} old login attempts`);
        }
      } catch (error) {
        // Don't fail the login if cleanup fails
        console.warn("Failed to cleanup old login attempts:", error);
      }
    }

    // Handle different transaction results
    if (result.type === "permanently_locked") {
      return Response.json(
        ...
        { status: 423 } // 423 Locked status code for permanent lockout
      );
    }
