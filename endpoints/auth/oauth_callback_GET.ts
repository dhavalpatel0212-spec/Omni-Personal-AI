...
export async function handle(request: Request) {
  try {
    ...
    if (existingUsers.length > 0) {
      ...
      // Check waiting list settings before creating new user
      const waitingListSettings = await db
        .selectFrom("waitingListSettings")
        .select(["waitingListEnabled", "maxActiveUsers"])
        .orderBy("createdAt", "desc")
        .limit(1)
        .executeTakeFirst();

      if (waitingListSettings?.waitingListEnabled) {
        console.log("Waiting list is enabled, performing checks");
        ...
        const userCount = Number(currentUserCount?.count || 0);
        const maxUsers = waitingListSettings.maxActiveUsers || 0;

        if (userCount >= maxUsers) {
          console.log("Max users limit reached", { userCount, maxUsers });
          ...
          const errorMessage: OAuthErrorMessage = {
            ...
            provider: oauthState.provider as OAuthProviderType,
            error: {
              message: "Registration is currently closed. We've reached our maximum capacity.",
        ...
        // Check if user email is approved in waiting list
        const waitingListEntry = await db
          .selectFrom("waitingList")
        ...
        if (!waitingListEntry) {
          console.log("User email not found in waiting list", mappedUserData.email);
          ...
          const errorMessage: OAuthErrorMessage = {
            ...
            provider: oauthState.provider as OAuthProviderType,
            error: {
              message: "You need to join our waiting list before you can register. Please visit our website to request access.",
        ...
        if (waitingListEntry.status !== "approved" || !waitingListEntry.approvedAt) {
          console.log("User not approved in waiting list", { 
          ...
          const errorMessage: OAuthErrorMessage = {
            ...
            provider: oauthState.provider as OAuthProviderType,
            error: {
              message: "Your registration request is still pending approval. We'll notify you once you're approved to join.",
              code: "waiting_list_not_approved",
              details: mappedUserData.email,
            },
          };
          return new Response(createOAuthPopupResponseHtml(errorMessage), {
        ...
        console.log("User approved in waiting list, proceeding with registration");
      }
